...
def rotate_cut_img(im, box, leftAdjustAlph=0.0, rightAdjustAlph=0.0):
    ...
    box = (max(1, cx - w / 2 - leftAdjustAlph * (w / 2))  ##xmin
           , max(1, cy - h / 2 - leftAdjustAlph * (h / 2)),  ##ymin
           min(cx + w / 2 + rightAdjustAlph * (w / 2), im.size[0] - 1)  ##xmax
           , min(cy + h / 2 + rightAdjustAlph * (h / 2), im.size[1] - 1))  ##ymax
    newW = box[2] - box[0]
    newH = box[3] - box[1]
...
def union_rbox(result, alpha=0.1):
    ...
    def diff(box1, box2):
        ...
        计算box1,box2之间的距离
        """
        cy1 = box1['position']['top']
        cy2 = box2['position']['top']
        h1 = box1['position']['height']
        h2 = box2['position']['height']
        if str(cy1) == '639.0' or str(cy1) == '629.0':
            print('===========')
            print(abs(cy1 - cy2) / max(0.01, min(h1 / 2, h2 / 2)))
    ...
    def sort_group_box(boxes):
        ...
        """
        N = len(boxes)
        boxes = sorted(boxes, key=lambda x: x['position']['left'])
        text = ' '.join([bx['words'] for bx in boxes])
        box4 = np.zeros((N, 8))
        for i in range(N):
            cx = boxes[i]['position']['left']
            cy = boxes[i]['position']['top']
            #            degree =boxes[i]['degree']
            degree = 0
            w = boxes[i]['position']['width']
            h = boxes[i]['position']['height']
        ...
        scores = [result['score'] for result in boxes]
        score = sum(scores) / len(scores)
        return {'words': text, "position": {'left': cx, 'top': cy, 'width': w, 'height': h}, 'degree': 0,
                'score': score}
...
def union_rbox2(result, alpha=0.1):
    """
    按行合并box
    """
